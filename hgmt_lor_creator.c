...
double bilinear_interp(double arr[][COLS], int num_rows, int num_cols, double i_full, double j_full) {
    ...
    if ((i == 0 && j == 0) || (i == num_rows-1 && j == 0) || (i == 0 && j == num_cols-1) || (i == num_rows-1 && j == num_cols-1)) {
        interpolated_val = arr[i][j];
    // checks if on top  or bottom side
    } else if (i == 0 || i == num_rows-1) {
        prefactor = 1.0/(dj_r - dj_l);
        interpolated_val = prefactor*(arr[i][j_l]*(dj_r-j_full) + arr[i][j_r]*(j_full-dj_l));
    // checks if on left or right side
    } else if (j == 0 || j == num_cols-1) {
        prefactor = 1.0/(di_r - di_l);
        interpolated_val = prefactor*(arr[i_l][j]*(di_r-i_full) + arr[i_r][j]*(i_full-di_l));
    // handles point being in the center of the rectangle
    } else {
        prefactor = 1.0/(di_r-di_l)*(dj_r-dj_l);
        double term_1 = (di_r-i_full)*(arr[i_l][j_l]*(dj_r-j_full) + arr[i_l][j_r]*(j_full-dj_l));
        double term_2 = (i_full-di_l)*(arr[i_r][j_l]*(dj_r-j_full) + arr[i_r][j_r]*(j_full-dj_l));
...
void test_bl_interp() {
    ...
    printf("i=30.1, j=30.1 -> %lf\n", bilinear_interp(eff_by_ang, ROWS, COLS, 30.1, 30.1));     // correct (exact)
    printf("Checking sides:\n");
    printf("i=0.1, j=5.48 -> %lf\n", bilinear_interp(eff_by_ang, ROWS, COLS, 0.1, 5.48));       // correct (in range)
    printf("i=2.241, j=0.1 -> %lf\n", bilinear_interp(eff_by_ang, ROWS, COLS, 2.241, 0.1));     // correct (in range)
    printf("i=30.1, j=5.48 -> %lf\n", bilinear_interp(eff_by_ang, ROWS, COLS, 30.1, 5.48));     // correct (in range)
    printf("i=2.241, j=30.1 -> %lf\n", bilinear_interp(eff_by_ang, ROWS, COLS, 2.241, 30.1));   // correct (in range)
    printf("Checking center:\n");
    printf("i=2.241, j=5.48 -> %lf\n", bilinear_interp(eff_by_ang, ROWS, COLS, 2.241, 5.48));   // correct (in range)
...
event* read_event(FILE* source) {
	...
	int dir_cos_neg; // 9: Flag to tell if Third Direction Cosine is Negative (1 means true)
	int first_in_hist; // 10: Flag to tell if this is the First Scored Particle from this History (1 means true)
...
event* get_event_pair(FILE* source, event* (*f)(FILE*)) {
    ...
    while(found_pair) {
        ...
        if ((first_event->run_id != second_event->run_id) || (first_event->event_id != second_event->event_id) || (first_event->particle != 22) || (second_event->particle != 22)) {
            free(first_event);
    ...
    // orders the events such that event_pair[0] arrived first and event_pair[1] arrived second
...
void print_event_pair(event* pair){
    ...
    for (int i = 0; i<2; i++){
        ...
        printf("(x,y,z): (%lf, %lf, %lf) cm, TOF: %lf, Run ID: %u, History ID: %u, Particle in History ID: %u\n", single_event.location.x, single_event.location.y, single_event.location.z, single_event.tof, single_event.run_id, single_event.event_id, single_event.track_id);
    }
...
int is_gamma_detected(event* single_event) {
    ...
    // converts the flag for the sign of the third direction cosine into a factor of 1 or -1
    ...
    // dir is the momentum unit vector of the gamma; has to check that the square root is not imaginary (happens with floats)
    ...
    double efficiency = bilinear_interp(eff_by_ang, ROWS, COLS, phi_ind, theta_ind);
...
prim_lor* create_primitive_lor(event* event_pair) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        // circ_i_hat is the normalized vector that is tangent to a circular cross section of the detector
        // btw, this is where the assumption that the detector is pointed along and is cocentric with the z-axis
        vec3d circ_i = vec_cross(loc_i, z_hat);
        vec3d circ_i_hat = vec_norm(circ_i);

        // creates three separate random numbers that are gaussian distributed about zero
        double i_z_rand = 0.0;
        double i_circ_rand = 0.0;
        double i_time_rand = 0.0;
        for (int j = 0; j < UNCERT_REP; j++) {
            i_z_rand += drand48();
            i_circ_rand += drand48();
            i_time_rand += drand48();
        }
        i_z_rand -= (float)UNCERT_REP / 2.0;
        i_circ_rand -= (float)UNCERT_REP / 2.0;
        i_time_rand -= (float)UNCERT_REP / 2.0;

        // creates vectors in the two distinct directions tangential to the detector with magnitudes a random fraction of the spatial uncertainty
        vec3d i_z_rand_vec = vec_scaler(z_hat, i_z_rand*SPC_UNC);
        // printf("z randomness: ");
        // vec_print(i_z_rand_vec, stdout);
        // printf("\n");
        vec3d i_circ_rand_vec = vec_scaler(circ_i_hat, i_circ_rand*SPC_UNC);
        // any uncertainty applied in the r_hat direction?

        // creates the new (blurred) location of the gamma interaction, and inserts it into new_prim_lor
        ...
        if (i==0) {
            ...
            new_prim_lor->first_time = gamma_i.tof + i_time_rand*TIME_UNC;
        } else{
            new_prim_lor->second_loc = new_i_loc;
            new_prim_lor->second_energy = gamma_i.energy;
            new_prim_lor->second_time = gamma_i.tof + i_time_rand*TIME_UNC;
        }
...
void test_create_primitive_lor(event* event_pair, prim_lor* primitive_lor) {
    ...
    printf("first gamma: \n");
    printf("(x,y,z) = (%lf, %lf, %lf) cm; TOF = %lf ns\n", loc_1.x, loc_1.y, loc_1.z, gamma_1->tof);
    printf("(x',y',z') = (%lf, %lf, %lf) cm; TOF' = %lf ns\n\n", loc_1_new.x, loc_1_new.y, loc_1_new.z, primitive_lor->first_time);

    printf("second gamma: \n");
    printf("(x,y,z) = (%lf, %lf, %lf) cm; TOF = %lf ns\n", loc_2.x, loc_2.y, loc_2.z, gamma_2->tof);
    printf("(x',y',z') = (%lf, %lf, %lf) cm; TOF' = %lf ns\n\n", loc_2_new.x, loc_2_new.y, loc_2_new.z, primitive_lor->second_time);
...
lor* create_lor(prim_lor* primitive_lor) {
    ...
    double longtidudinal_uncert = sqrt((SPD_LGHT*TIME_UNC)*(SPD_LGHT*TIME_UNC) + (SPD_LGHT*TIME_UNC)*(SPD_LGHT*TIME_UNC));
...
int main(int argc, char **argv) {
    ...
    if (argc == 2 && (!strcmp(argv[1], "-h") || !strcmp(argv[1], "--help"))) {
        ...
        printf("Usage: ./hgm_lor_creator [TOPAS_file_location.phsp] [efficiency_table_location.csv] [LOR_output_location]\n");
    ...
    while (pair_of_events != NULL) {
        // prints a status update every 1,000,000 gammas we read through (effectively the line number the code is on)
        if ((gammas_entered_scanner / 1000000) * 1000000 == gammas_entered_scanner) {
			printf("Number of recorded gammas read: %u\n", gammas_entered_scanner);
		}

        // works out if the pair of gammas that enter the detector are actually detected
        int pair_detected = 0;
        for (int i=0; i<2; i++) {
        ...
        // frees memory pointed to by pair_of_events and proceeds on to get next pair of events
        free(pair_of_events);
    ...
    // prints metadata
    printf("Gammas that entered the detector: %u\nGamma pairs that entered the detector: %u\nGamma pairs that were detected by the detector: %u\n", gammas_entered_scanner, gamma_pairs_entered_scanner, gamma_pairs_detected);
    uint one_ips_scatter = gamma_pairs_detected_atleast_one_ips - gamma_pairs_detected_both_ips;
    double percent_one_ips = 100.0*(double)one_ips_scatter/((double)(gamma_pairs_detected));
    printf("Detected gamma pairs with only one gamma doing in-patient scattering: %i (%.2f%%)\n", one_ips_scatter, percent_one_ips);
    double percent_both_ips = 100.0*(double)gamma_pairs_detected_both_ips/((double)(gamma_pairs_detected));
    printf("Detected gamma pairs with both gammas doing in-patient scattering: %i (%.2f%%)\n", gamma_pairs_detected_both_ips, percent_both_ips);
    double percent_atleastone_ips = 100.0*(double)gamma_pairs_detected_atleast_one_ips/((double)(gamma_pairs_detected));
    printf("Total number of detected gamma pairs with an in-patient scatter: %i (%.2f%%)\n", gamma_pairs_detected_atleast_one_ips, percent_atleastone_ips);
    if (DIAGNOSTIC_TOTAL_GAMMAS_DETECTED) {
        uint total_gammas_detected = 2*gamma_pairs_detected + diagnostic_single_gammas_detected;
        printf("Total gammas detected (including pairs and single gammas detected): %u\n", total_gammas_detected);
        double total_eff = 100.0*(double)total_gammas_detected/((double)gammas_entered_scanner);
