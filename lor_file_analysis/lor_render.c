...
// expects a lor file with: hist_num, c_x, c_y, c_z, d_x, d_y, d_z, sigma_l,
// sigma_t
...
void add_double(render *u, double x, int *indexes, double atten) {
  u->volume[indexes[0] * u->dimensions[1] * u->dimensions[2] +
            indexes[1] * u->dimensions[2] + indexes[2]] += x * atten;
  return;
}
...
void add_mult_log(render *u, double x, int *indexes, double arg) {
  u->volume[indexes[0] * u->dimensions[1] * u->dimensions[2] +
            indexes[1] * u->dimensions[2] + indexes[2]] += x;
  u->volume[indexes[0] * u->dimensions[1] * u->dimensions[2] +
            indexes[1] * u->dimensions[2] + indexes[2]] +=
      log((u->volume[indexes[0] * u->dimensions[1] * u->dimensions[2] +
                     indexes[1] * u->dimensions[2] + indexes[2]]) *
              x +
          1.0);
  return;
...
void add_binary(render *u, double x, int *indexes, double atten) {
  u->volume[indexes[0] * u->dimensions[1] * u->dimensions[2] +
            indexes[1] * u->dimensions[2] + indexes[2]] += 1.0 * atten;
  return;
}
...
void add_log(render *u, double x, int *indexes, double arg) {
  u->volume[indexes[0] * u->dimensions[1] * u->dimensions[2] +
            indexes[1] * u->dimensions[2] + indexes[2]] += log(x + 1);
  return;
}
...
void low_high_corner(vec3d *corner1, vec3d *corner2, vec3d *low_corner,
                     vec3d *high_corner) {
  if ((corner1 == NULL) || (corner2 == NULL) || (low_corner == NULL) ||
      (high_corner == NULL)) {
    return;
  }
...
render *read_render_def(FILE *input) {
  if (input == NULL) {
    fprintf(stderr,
            "read_render_def: null pointer given to render definitions");
    return NULL;
  }
  double x, y, z;
  int worked;
  ...
  int a, b, c;
  worked = fscanf(input, "%i,%i,%i", &a, &b,
                  &c); // gets the number of voxels in each dimension
...
geometry *read_geometry(FILE *input) {
  if (input == NULL) {
    fprintf(stderr,
            "WARN: no geometry found, continuing without attenuation\n");
    return NULL;
  }
  char type[64];
  int worked;
...
void coord_transfer(double *transfer, int *spots) {
  if ((spots[0] == spots[1]) || (spots[0] == spots[2]) ||
      (spots[1] == spots[2])) {
...
vec3d lor_box_offset(lor *lor, double cutoff_sigma) {
  ...
  trans_dir.x =
      (lor->dir.x > 0) -
      (lor->dir.x < 0); // 1 if heading in x dir, -1 if in -x dir 0 if 0
  trans_dir.y = (lor->dir.y > 0) - (lor->dir.y < 0);
  trans_dir.z = (lor->dir.z > 0) - (lor->dir.z < 0);
  vec3d trans_vec =
      vec_scaler(trans_dir, cutoff_sigma * lor->transverse_uncert);
...
double centered_normal(double x) {
  ...
  // // if this is our first run of the centered normal function we need to
  // build
...
double centered_binary(double x) { return 1.0; }
      ...
      for (int k = low_z; k <= high_z; k++) {
        ...
        if ((longitudinal < (universe->cutoff * lor->long_uncert)) &&
          ...
          if ((universe->cutoff * universe->cutoff) >
              (lon_deviation * lon_deviation) +
                  (trans_deviation * trans_deviation)) {
            // we are within the processing column (area of useful adding
            // values)
...
void add_lor_plane(render *universe, lor *lor) {
  ...
  for (int i = low_x; i <= high_x; i++) {
    ...
    double x_dist =
        (i / universe->conversion.x) + universe->least_corner.x - lor->center.x;
    double displacement =
        x_dist / lor->dir.x; // need to make sure it is not a divide by 0
  ...
  for (int i = 0; i < universe->dimensions[0]; i++) {
    for (int j = 0; j < universe->dimensions[1]; j++) {
      for (int k = 0; k < universe->dimensions[2]; k++) {
        fprintf(output, "%lf ",
                universe->volume[i * universe->dimensions[1] *
                                     universe->dimensions[2] +
                                 j * universe->dimensions[2] + k]);
      }
      fprintf(output, "\n");
    }
    // fprintf(output, "\n");
...
int main(int argc, char const *argv[]) {
  ...
  if (argc < 4) {
    ...
    printf("Expected 3 inputs, a lor file name, a rendering defintion file "
           "name and");
  ...
  FILE *input_lor = fopen(argv[1], "rb");
  if (input_lor == NULL) {
  ...
  for (int i = 0; i < MAX_THREADS; i++) {
    ...
    if (pthread_mutex_init(&(working_lors[i].lock), NULL) &&
        pthread_mutex_init(&(working_lors[i].hold), NULL)) {
      fprintf(stderr, "pthread: unable to make update lock, exiting.\n");
      return (1);
    }
    pthread_create(&tid[i], NULL, long_life_thread_add_lor_plane,
                   &(working_lors[i]));
  }
  ...
  while (operative_lor != NULL) {
      ...
      for (int i = 0; (i < MAX_THREADS) && (cont == 0); i++) {
        ...
        if (!working_lors[i].flag) {
          ...
          if (DEBUG_THREADS_PRINT)
            fprintf(stderr, "main waking thread %i\n", i);
          pthread_cond_signal(
              &(working_lors[i].cond)); // rewake the thread if it is stuck
        }
    ...
    // struct _add_lor_union *arguments = (struct _add_lor_union
    // *)malloc(sizeof(struct _add_lor_union)); arguments->universe =
    // master_copy; arguments->lor = operative_lor;
  ...
  printf("LORs not added:\n\t%li, fractionally %lf\n", not_added,
         (double)not_added / (double)iterated);
