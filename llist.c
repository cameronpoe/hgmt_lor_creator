...
llist *add_to_top(llist *list, event *data) {
...
llist *rec_add_to_bottom(llist *list, event *data) {
  // does the list even exist yet?
  if (list == NULL) {
    // ok time to make a list
    list = (llist *)malloc(sizeof(llist));
...
llist *iter_add_to_bottom(llist *list, event *data) {
...
// adds a piece of data to the end of a list
llist *add_to_bottom(llist *list, event *data) {
  return iter_add_to_bottom(list, data);
}

int rec_wipe_list(llist *list) {
  ...
  if (list->data != NULL) {
    ...
    // printf(stderr, "delete_list error: deleted list not NULL\n");
  }
  int success = rec_wipe_list(list->down);
  free(list);
  return success;
}
// deletes an empty list, cannot clear the data
int wipe_list(llist *list) {
  if (list == NULL) {
    return 1;
  }
  list = list_head(list);
  return rec_wipe_list(list);
}
